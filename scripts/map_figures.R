...
pit_tracks <-read.csv("~/Desktop/Chapter1/MESOSCOPE2017/Data/pit_tracks.csv", header=T)
...
pit_points<-read.csv("~/Desktop/Chapter1/MESOSCOPE2017/Data/pit_points.csv", header=T)
...
stn_points <-read.csv("~/Desktop/Chapter1/MESOSCOPE2017/Data/station_points.csv", header=T)
stn_points$Station<-factor(as.character(stn_points$Station),levels=c("S4","S6","S8","S10","S12","S14","Stn. ALOHA"))

# input sla information
sla_df_lat <- read.csv("~/Desktop/Chapter1/MESOSCOPE2017/Data/SLAcorr_20170701.csv", header=F) 
sla_long <- read.csv("~/Desktop/Chapter1/MESOSCOPE2017/Data/SLA_long.csv", header=F)
...
subset_map<-ggplot() +
  ...
  geom_point(data=stn_points, aes(x=Long,y=Lat), color="black",shape=c(21,21,21,21,21,21,8),size=4,stroke=1)+
  geom_text(data=stn_points,aes(x=Long-.15, y=Lat+0.12,label=Station),color="black")+
  theme_bw()+
  ylab("")+
  xlab("")+
...
plot.with.inset <-
  ...
  draw_plot(subset_map, x = .6, y = .35, width = 0.4, height = 0.5)
plot.with.inset
