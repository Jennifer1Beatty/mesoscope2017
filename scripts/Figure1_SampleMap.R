...
hawaii_map<-ggplot()+
  ...
  scale_x_continuous(limits=c(-162,-152),breaks=c(-162,-160,-158,-156,-154,-152),label=c("162°W","160°W","158°W","156°W","154°W","152°W"))+
  scale_y_continuous(limits=c(18,28), breaks=c(18,20,22,24,26,28),label=c("18°N","20°N","22°N","24°N","26°N","28°N"))+
  geom_circle(data=stn_ALOHA,aes(x0=-Long,y0=Lat,r=.1), color="black")+ # adds a circle that is .1 of a degree or 6 nmi which is definition of Station ALOHA
  geom_rect(aes(ymin = 23.25, ymax = 28, xmin = -156.5, xmax = -160.5,),alpha = .2,color="black")+
  theme_bw()+
  ylab("")+
  xlab("")+
  theme(panel.grid.minor = element_line(colour="white"), panel.grid.major = element_line(linetype=2,colour = "grey"), axis.ticks = element_blank())
hawaii_map
...
subset_map<-ggplot() +
  ...
  scale_x_continuous(trans="reverse",limits=c(160.5,156.5),breaks=c(160,159,158,157),label=c("160°W","159°W","158°W","157°W"),position = "bottom")+
  scale_y_continuous(limits=c(23.25,28), breaks=c(24,25,26,27,28),label=c("24°N","25°N","26°N","27°N","28°N"),position = "right")+
  ...
  geom_text(data=pit_points,aes(x=Long-.1, y=Lat,label=Station),color="black", size=2)+
  ...
  theme(panel.grid.minor = element_line(colour="white"), panel.grid.major = element_line(colour="white"),axis.ticks = element_blank())
subset_map
...
plot.with.inset <-
  ...
  draw_plot(subset_map, x = .5, y = .35, width = .6, height = .6)
plot.with.inset

# ggsave(filename = "~/github/mesoscope2017/figures/Figure1_Map.pdf", plot = plot.with.inset, width = 5, height = 6,units = "in",dpi = 300)
