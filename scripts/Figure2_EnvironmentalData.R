# Figure 2- Depth profile of environmental parameters
# J. L. Beatty 
# Last updated: 05/24
...
head(metadata_fr)
# subset from total dataset, rename the columns and take the mean if necessary per depth
...
# prepare for plotting, need to organize by depth per station
...
# determined the appropriate tone for each station based on a gradient 
...
# run the data through the plotting function
plots <- meta_plot(metadata.m)
unique(metadata.m$variable) # identify the location of variables of interest
# subset the plots I'm interested in
(plots[[5]] + plots[[8]] + plots[[10]] + plots[[11]])+ plot_annotation(tag_levels = "a")
# ggplot2::ggsave("~/github/mesoscope2017/figures/Figure2.pdf", width=5, height = 6, units="in", dpi=300)
